function [t,v0,i0,z,fcorr,Vp] = timevariant_EIS_v7_beta(A,B,Dt,DT,CA_range,f,filter,bw,n,cal,mirrorRequest)
...
% I am isolating the mirroring procedure. Alberto
...
if Nt ~= length(B)
    error('wrong data set')
    %return
end
if Ni < 1
    error('time sampling too short')
    %return
end
if mod(Nt,2) ~= 0
...
fraux = (1/Dt)*linspace(-0.5,0.5-1/lfctr/Nt,Nt*lfctr)';
fraux1 = (1/DT)*linspace(-0.5,0.5-0.5/Np,Np*lfctr)';
fpt0 = ceil(Nt/2)+1;
...
Vaux = fftshift(fft(vaux))/(lfctr*Nt);
Iaux = fftshift(fft(iaux))/(lfctr*Nt);
if mirrorRequest == true
    range = lfctr*fpt0-Np-1:lfctr*fpt0+Np-2;
    v0l = (ifft(ifftshift(Vaux(range).*fil(fraux(range),0,bw,n)),'symmetric').')*lfctr*Np;
    i0l = (ifft(ifftshift(Iaux(range).*fil(fraux(range),0,bw,n)),'symmetric').')*lfctr*Np;
    v0 = v0l(1,1:Np);
    i0 = i0l(1,1:Np);
else
    range = lfctr*fpt0-Np/2:lfctr*fpt0+Np/2-1;
    v0 = (ifft(ifftshift(Vaux(range).*fil(fraux(range),0,bw,n)),'symmetric').')*lfctr*Np;
    i0 = (ifft(ifftshift(Iaux(range).*fil(fraux(range),0,bw,n)),'symmetric').')*lfctr*Np;
end

% find the correct frequencies of the perturbation by using the data set A
    
v = vaux - (ifft(ifftshift(Vaux.*fil(fraux,0,bw,n)),'symmetric'))*lfctr*Nt;
if mirrorRequest == true
    v(length(A)+1:2*length(A),:) = [];
end
V = fftshift(fft(v))/Nt;
i = iaux - (ifft(ifftshift(Iaux.*fil(fraux,0,bw,n)),'symmetric'))*lfctr*Nt;
if mirrorRequest == true
